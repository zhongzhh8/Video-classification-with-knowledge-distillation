...
if __name__ == "__main__":
    # Dataset flags.
    flags.DEFINE_string("train_dir", "nextvlad_8g_5l2_5drop_128k_2048_2x80_logistic",
                        "The directory to load the model files from. "
                        "The tensorboard metrics files are also saved to this "
                        "directory.")
    flags.DEFINE_string(
        "eval_data_pattern", "dataset/validate/validate*.tfrecord",
        "File glob defining the evaluation dataset in tensorflow.SequenceExample "
        "format. The SequenceExamples are expected to have an 'rgb' byte array "
        "sequence feature as well as a 'labels' int64 context feature.")

    # Other flags.
    flags.DEFINE_integer("batch_size", 80,
...
def build_graph(reader,
    ...
    with tf.variable_scope("tower"):
        representation, result = model.create_model(model_input,
...
def evaluation_loop(video_id_batch, prediction_batch, label_batch, loss,
    ...
    with tf.Session() as sess:
        ...
        #
        #     # Save model  暂时不管这个破interference了
        ...
        saver.restore(sess, checkpoint_name)
        global_step_val = os.path.basename(checkpoint_name).split("-")[-1]
        ...
        try:
            ...
            epochinfo ,EvalGAP= utils.AddEpochSummary(
                summary_writer,
                global_step_val,
                epoch_info_dict,
                summary_scope="Eval")
        ...
        return global_step_val,EvalGAP
...
def evaluate(model,checkpoint_name,k_frame):
    ...
    with tf.Graph().as_default() as graph:
        ...
        while True:
            last_global_step_val ,EvalGAP= evaluation_loop(video_id_batch, prediction_batch,
